...
| Version | Closed |
|---|---|
| v276 | **`NMX_ZSYNC` back on — the vector renderer had been 6.4× out of step.** Device numbers at v275: Container scale worst **6.372** at z9.97, last 3.577. Leaflet's zoom scale is 2^Δz, so that is the SVG renderer sitting 2.67 zoom levels behind the map with a CSS scale covering the gap, multiplying every sub-pixel error by up to 6.4 and fattening the stroke with it. The flag was switched off in v269 to test the two-clock theory; that theory died in v270 when the floating turned out to be `.rotfix`, and I left it off for five more builds. Cost objection answered by measurement: re-project worst **4 ms**, not the 13 ms from v253 (which predated removing the view-change rebuild). **Marker theory dead**: v275's toggle reported **18 icons / 89 DOM nodes**, not the ~800 I had argued, and the pinch still jiggled with them off. |
| v275 | **Markers-off diagnostic toggle** (More → Diagnostics → Markers). Removes the marker LayerGroup from the map so a pinch can be measured with a finger rather than argued about; the panel also counts icons on the map and the DOM nodes they carry (each marker is a wrapper span, a `.fld` div, a dot and a label, so nodes ≈ 4× icons). Smooth with markers off → the DOM marker layer is the cost and the fix is a canvas marker renderer. Still floaty → the cost is in the tile renderers and the answer is a different map engine. Also: **scenario audit** (`audit.js`) — ~150 seeded random interactions on a loaded route found zero throws, zero NaN/undefined, zero dead controls. |
| v274 | **The focus checks its own result.** v273 put a tapped airfield at CSS **94.0, 147.8** against a target of **187.5, 286.5**; the same airfield with the card already open landed at **185.0, 284.8**, inside the 4 px deadband — so the geometry was right and measuring it mid-slide was not. Rather than guess the cause from two stills, the open loop became a closed one: aim, pan, settle, **measure again**, correct. Bounded at four passes, abandoned on a user drag, with a generation counter so a newer tap retires an older loop. `focus.js` now runs the whole function with a synchronous rAF and a `panBy` that moves the simulated subject, and carries a scenario that reproduces the fault — it reports `1 passes` against v273 and `2 passes` against v274. **Recording is red** on both rails; the landscape HUD already was, so the same control was two colours depending on how the phone was held. |
| v273 | **Tap a point, get a half-screen card with the point centred in the map above it.** Most of the machinery existed — `mapFocus()` trims the map rect by every overlay and pans the subject into what is left. Three gaps: the measurement **raced the 220 ms slide-up** (two rAFs, and `getBoundingClientRect` reports the transformed box, so almost nothing was trimmed — it only looked right when the card was already open); the card height was **whatever was last dragged**, so the "upper half" was a different shape each time; and **route points with no DB entry** opened the route-actions sheet without centring. Now: waits for the overlay rects to stop changing (400 ms ceiling), map taps open at half the window, and every point opens a card. |
